...
- **`content` (관련 콘텐츠 원문)**: `targetType`에 따라 다르게 채워집니다.
  ...
  - `CHAT`: **TODO 미구현** — 채팅방 존재 여부만 확인하고 실제 메시지 원문은 아직 안 가져옵니다(신고 시점 스냅샷 vs 실시간 조회 정책 미정). 채팅방이 없으면 `"삭제된 콘텐츠입니다"`, 있으면 임시로 `"TODO: 채팅 로그 연동 필요"` 문자열을 반환합니다.
...
| CHAT 유형 관련 콘텐츠(채팅 로그) 원문 조회 | 미구현 — placeholder 문자열만 반환 |
