...
public class ProjectReqDTO {
        ...
        public record CreateProjectReq(
                @Schema(description = "프로젝트 분야 (WEB, MOBILE, AI, GAME, DATA, BACKEND, FRONTEND)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "프로젝트 분야는 필수입니다.")
                ProjectField projectField,

                @Schema(description = "카테고리(도메인) ID", requiredMode = Schema.RequiredMode.REQUIRED, example = "1")
                @NotNull(message = "도메인은 필수입니다.")
                @Min(value = 1, message = "카테고리 ID는 1 이상이어야 합니다.")
                Long categoryId,

                @Schema(description = "진행 방식 (ONLINE, OFFLINE, HYBRID)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "진행 방식은 필수입니다.")
                ProjectMode mode,

                @Schema(description = "진행 기간 (개월 단위, 예: 2)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "진행 기간은 필수입니다.")
                @Min(value = 1, message = "진행 기간은 최소 한 달 이상이어야 합니다.")
                @Max(value = 13, message = "진행 기간은 최대 13개월 까지 가능합니다.")
                Integer durationMonths,

                @Schema(description = "진행 장소 (ONLINE인 경우 '온라인', OFFLINE인 경우 구체적 장소)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotBlank(message = "진행 장소는 필수입니다.")
                @Size(max = 100, message = "진행 장소는 100자를 초과할 수 없습니다.")
                String location,

                @Schema(description = "모집 마감일 (형식: YYYY-MM-DD)", requiredMode = Schema.RequiredMode.REQUIRED, example = "2026-01-25")
                @NotNull(message = "모집 마감일은 필수입니다.")
                LocalDate recruitmentDeadline,

                @Schema(description = "프로젝트 시작 예정일 (형식: YYYY-MM-DD)", requiredMode = Schema.RequiredMode.REQUIRED, example = "2026-02-01")
                ...
                @Schema(description = "프로젝트 내용", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotBlank(message = "프로젝트 내용은 필수입니다.")
                @Size(max = 2000, message = "프로젝트 내용은 2000자를 초과할 수 없습니다.")
                String content,

                @Schema(description = "프로젝트 이미지 URL 목록", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
        ...
        public record UpdateProjectReq(
                ...
                @Schema(description = "진행 기간 (개월 단위)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "진행 기간은 필수입니다.")
                @Min(value = 1, message = "진행 기간은 최소 1개월 이상이어야 합니다.")
                @Max(value = 13, message = "진행 기간은 최대 13개월까지 가능합니다.")
                Integer durationMonths,

                @Schema(description = "진행 장소", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotBlank(message = "진행 장소는 필수입니다.")
                @Size(max = 100, message = "진행 장소는 100자를 초과할 수 없습니다.")
                String location,
                ...
                @Schema(description = "프로젝트 시작 예정일", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "프로젝트 시작일은 필수입니다.")
                LocalDate startDate,
        ...
        public record RecruitmentDTO(
                @Schema(description = "포지션 (BACKEND, FRONTEND, DESIGN, PM, IOS, ANDROID)", requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull(message = "포지션은 필수입니다.")
                ProjectPart position,

                @Schema(description = "모집 인원 (1~10명)", requiredMode = Schema.RequiredMode.REQUIRED)
