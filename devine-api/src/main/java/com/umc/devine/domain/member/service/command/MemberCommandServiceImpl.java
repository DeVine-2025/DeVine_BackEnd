...
import com.umc.devine.domain.member.event.MemberSignedUpEvent;
...
public class MemberCommandServiceImpl implements MemberCommandService {
    ...
    public MemberResDTO.SignupResultDTO signup(ClerkPrincipal principal, MemberReqDTO.SignupDTO dto) {
        ...
        // 11. 초기 리포트 생성권 지급 (트랜잭션 커밋 후 이벤트로 처리)
    ...
    public MemberResDTO.MemberProfileDTO updateMember(Member member, MemberReqDTO.UpdateMemberDTO dto) {
        ...
        // 멤버 업데이트
        java.util.Optional.ofNullable(dto.nickname()).ifPresent(member::updateNickname);
        java.util.Optional.ofNullable(dto.imageUrl()).ifPresent(member::updateImage);
        java.util.Optional.ofNullable(dto.address()).ifPresent(member::updateAddress);
        java.util.Optional.ofNullable(dto.body()).ifPresent(member::updateBody);
        java.util.Optional.ofNullable(dto.mainType()).ifPresent(member::updateMainType);
        java.util.Optional.ofNullable(dto.disclosure()).ifPresent(member::updateDisclosure);
        java.util.Optional.ofNullable(dto.proposalAlarm()).ifPresent(member::updateProposalAlarm);
